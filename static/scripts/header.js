          ...
          '<div class = "product-item d-flex flex-row" id = "productItem'+id+'">'+
               '<div class = "d-flex flex-row product-link flex-grow-1">'+
                    '<div class = "flex-fill image-container">'+
                         '<a href = "/obp/product-'+id+'/" ><img src = "' + img_url + '"></a>'+
                    '</div>'+
               ...
               '<div class = "d-flex flex-row product-buttons flex-grow-1">'+
                    '<div class="btn-toolbar" role="toolbar" aria-label="Toolbar with button groups">'+
                         '<div class="btn-group mr-2" role="group" aria-label="First group">'+
                              '<button type="button" class="basket-btn btnPlusQuantity nonOutline" onClick = "plusQuantity('+id+')">+</button>'+
                              '<div class="quantity align-middle"><span id = "quantityField'+id+'">'+ quantity +'</span></div>'+
                              '<button type="button" class="basket-btn nonOutline" onClick = "minusQuantity('+id+')">-</button>'+
                         '</div>'+
                    '</div>'+
                    '<div class = "btn-delete">'+
                         '<button type="button nonOutline"  onClick = "deleteProduct('+id+')">&times;</button>'+
                    '</div>'+
               '</div>'+
          '<div>';
     return product_item;
...
$(document).ready(function () {
     ...
     var div = $(".basket-block"); // тут указываем ID элемента
     ...
     $.ajax({
         type: 'GET',
         async: true,
         url: 'http://127.0.0.1:8000/cart/CartCount/',
         dataType: 'json',
     ...
     $("#btn-logout").click(function(){
          ...
          $.ajax({
              type: 'GET',
              async: false,
              url: 'http://127.0.0.1:8000/obp/logout/',
              success: function(data){
                  $("#auth-btn-login").removeClass("d-none");
                  $("#auth-btn-logout").addClass("d-none");
            },
              dataType: 'json',
          });
     });
